...
function maincardHTML(&$card,$row) {
	...
	// rest of footer, timer contains the return time
	if ($row['timer'] != 0) {
		$footer = "Return by " . $row['timer'] . ".";
	} else {
		$footer = ($row['favorite'] == 0) ? "Tap for more options." : $row['favorite'];
	}
...
function maincard($user_id) {	
	...
	// reading aloud if there is a note
	if ($row['notes'] != 0) {
...
if ($request['userActions'][0]['type'] == "SHARE") {
	...
	switch ($request['userActions'][0]['payload']) {
		...
		// occurs if user wishes to add a picture or a timer
		case 'pic':
		case 'timer':
		
			// create a new timeline card
			$picTimer = new Google_TimelineItem();
			
			// if picture provide instructions, if timer provide error message
			$message = ($request['userActions'][0]['payload'] == 'pic') ? 
			"Please swipe backwards and take a picture with Glass's camera. Then share with Parkifying Glass and it will be added to your location card." 
			: "Sorry, the timer is not yet supported.";
			
			// set text of new card
			$picTimer->setText($message . "\n\nTap twice to delete this card.");
			
			// array for menu item
			$menu_items = array();
			
			// menu item that deletes the temporary message
			$delete_menu_item = new Google_MenuItem();
			$delete_menu_item->setAction("DELETE");
			array_push($menu_items, $delete_menu_item);

			// set menu item
			$picTimer->setMenuItems($menu_items);

			// default notification
			$notification = new Google_NotificationConfig();
			$notification->setLevel("DEFAULT");
			$picTimer->setNotification($notification);
			
			// insert card
			insert_timeline_item($mirror_service, $picTimer, null, null);
		...
		case 'favorites':
			...
			if (counter) {
				...
				foreach ($rows_sorted as $row) {
					...
					// must navigate directly from sub-timeline item because if added a separate step, no way to tell which favorite from the current table
				...
				// else our menu item will be simply a message, so it needs a menu item that deletes the temporary message
				$menu_items = array();
				$delete_menu_item = new Google_MenuItem();
				$delete_menu_item->setAction("DELETE");
				array_push($menu_items, $delete_menu_item);
