...
function maincardHTML(&$card,$row) {
	...
	if ($row['timer'] != 0) {
		...
		// if user has set a timer, first calculate number of minutes (truncated)
		$minuteDifference = floor(($row['timer'] - time()) / 60);
		
		if (!$minuteDifference) {
			
			// if truncated number of minutes is 0 user should be there NOW
			$footer = "0 minutes left. Return ASAP!";
		
		} else {
			
			// else the time difference is not 0. first determine message head based on whether number of minutes is negative
			$messageHead = ($minuteDifference > 0) ? "Return in " : "Late by ";

			// then determine number of hours remaining
			$minuteDifference = abs($minuteDifference);
			$hourDifference = floor($minuteDifference / 60);

			// next determine number of minutes not accounted for in the number of hours
			$minuteDifference %= 60;

			// finally calculate message text
			$hourDisplay = ($hourDifference) ? $hourDifference . (($hourDifference == 1) ? " hour" : " hours") : "";
			$betweenDisplay = ($hourDifference && $minuteDifference) ? " " : "";
			$minuteDisplay = ($minuteDifference) ? $minuteDifference . (($minuteDifference == 1) ? " minute." : " minutes.") : ".";
			$footer = $messageHead . $hourDisplay . $betweenDisplay . $minuteDisplay;
		}
	
	} else {
		
		// else if user has a favorite, display the favorite description. if not, instruct how to use menu items
		$footer = ($row['favorite'] != 0) ? $row['favorite'] : "Tap for more options.";
	}
...
function maincard($id) {	
	...
	// ternary operator not an option here due to the multiple lines
	if ($row['favorite'] == 0) {
...
function navcard(&$navigation_timeline_card,$user_id) {
	...
	// current id and row
	$id = query("select id from current where userid = ?",$user_id)[0];
	$current_row = query("select * from location where id = ?", $id);
	...
	switch ($request['userActions'][0]['payload']) {
		...
		case 'remember':
			...
			if (!duplicate) {
				...
				$id = query("select id from locations where userid = ? order by id desc",$user_id)[0];
				store_current($id,$user_id);
			}
		...
		case '42': case '45': case '48': case '51': case '54': case '57': case '60':
			...
			// get return time, time() returns current time
			$returnTime = time() + $minutesToAdd * 60;
			
			// store return time in database
			$id = query("select id from current where userid = ?",$user_id)[0];
			query("update location set timer = ? where id = ?",$returnTime,$id);
